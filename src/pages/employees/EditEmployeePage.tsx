...
import { useEmployee, useUpdateEmployee, useCadres } from '@/hooks/useApi';
...
export default function EditEmployeePage() {
  ...
  useEffect(() => {
    if (employee) {
      reset({
        ...
        cadre_id: employee.cadre_id.toString(),
        ...
        annual_rent_paid: employee.annual_rent_paid
          ? koboToNaira(employee.annual_rent_paid)
          : '',
      });
    }
  }, [employee, reset]);
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 mb-6">Edit Employee</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-2xl">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Select
              ...
              options={(cadres || []).map((c) => ({ value: c.id.toString(), label: c.name }))}
              {...register('cadre_id')}
            />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Select
              ...
              options={NIGERIAN_BANKS.map((b) => ({ value: b.name, label: `${b.name} (${b.code})` }))}
              {...register('bank_name', {
                onChange: (e) => {
            ...
            <Input label="Account Number" error={errors.bank_account_number?.message} {...register('bank_account_number')} />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input label="Tax Identification Number (TIN)" {...register('tin')} />
              <Input label="Pension RSA PIN" {...register('pension_rsa_pin')} />
              <Input label="NHF Number" {...register('nhf_number')} />
              <Input label="Annual Rent Paid (NGN)" {...register('annual_rent_paid')} />
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex gap-3">
          ...
          <Button type="submit" loading={updateEmployee.isPending} icon={<Save className="h-4 w-4" />}>
            Save changes
          </Button>
        </div>
      </form>
